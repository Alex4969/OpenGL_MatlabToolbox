...
classdef javacallbackmanager < handle %matlab.mixin.Copyable
...
%   if no argument, remove every callbacks
    ...
    methods(Sealed=true)
        ...
        function clb=getAvailableCallback(obj)
            clb=obj.callback_list;
        end        
        ...
        function setMethodCallbackWithSource(obj,source,target)
            obj.isValidCallback(target,1);
            obj.h.([target 'Callback']) = @(src,evt) source.(['cbk_' target])(src,evt);
        end
